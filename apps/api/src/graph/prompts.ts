...
Фактический контекст (из базы знаний):
{ragContext}
...
Фактический контекст (из базы знаний): {ragContext}
Ссылки компании для органичной интеграции: {companyLinks}

{styleGuide}

Примеры стиля из эталонных статей:
{styleExamples}

Требования:
- ОБЯЗАТЕЛЬНО: следуй метрикам и конструкциям из стилевого руководства выше
- Избегай паттернов из раздела «ИЗБЕГАТЬ»
...
3. Проверь техническую точность формулировок
4. Добавь плавные переходы между секциями
5. Убери повторы, длинноты, общие фразы
6. Проверь соответствие стилевому руководству: длина предложений, характерные конструкции, отсутствие запрещённых паттернов
